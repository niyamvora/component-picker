...
### Changed
- **The toolbar click now opens the dock and the side panel; the popup is gone.** Clicking the icon used to open a popup whose first button was "Pick on this page" — and a popup closes the moment you click the page, which is the one thing you always do next. The click now shows the in-page dock and opens the side panel beside it. Everything the popup held moved into the panel: the last capture's rendering and screenshot, Copy bundle, Download assets, recent picks, the library, the compare reference, the output options, viewports, your component inventory and the MCP bridge — with the settings sections collapsed by default so the capture stays the thing you see.

### Added
- **Compare reference is a dock button, not a hidden key.** `R` only set the reference while the copy toast was up — six seconds to notice a hint you were not looking for, on a keystroke with no visible state. There is now a compare button in the dock: click it to arm, pick the component to compare against, and it stays lit while every later pick on any tab or site carries the diff. Click again to clear. `R` still works.
- **The note box opens against the component it is about.** It appeared bottom-centre, a dialog about nothing in particular; it now opens beside the element with the element's name in its header, a ring drawn around the element, and an OK button rather than only a hidden Enter.
- **Drawer polish** (#95): every group says what it would hold rather than rendering a blank void; scrollbars inside the drawer are thin and dark instead of the page's default chrome; `prefers-reduced-motion: reduce` turns off the drawer's transitions (the easing curves still draw — they are static); long bodies and long visualizations share one 200px cap with a bottom fade that appears only when there really is more below; buttons stripped by `all:unset` get a visible focus ring back; and **Esc** now closes the drawer if it is open and disarms the picker otherwise, instead of doing both at once.
- **Drawer v2: four collapsible groups** (#94): after the last three stages the drawer held about thirty rows in one flat list. It is now **Copy** (prompt, output box, section checkboxes), **Design** (swatches, type, spacing, easing), **Capture options** (the output toggles) and **Extension** (dock, MCP bridge, library) — one open at a time, and the one you left open comes back next time. Each group's body is built on first expand, so a closed Design group costs nothing. Opens on **Copy** after a capture, **Capture options** before your first one. The visualizations moved out of the individual section rows into Design, where they read as one panel instead of hiding behind ten separate carets.
- **GSAP ScrollTrigger geometry** (#92): the `## GSAP` section now also carries each trigger whose trigger or pin is in the pick — `start "top 80%" · end "bottom 20%" · scrub · pins [data-cp="1"]`. On a scroll-driven site that geometry *is* the design, and it was the half the tween list could not describe. A start or end authored as a function is printed as the px it resolved to, marked `(resolved)`.
- **Canvas and WebGL scenes named, not silently dropped** (#91): a `<canvas>` in the pick now gets a `## Canvas / WebGL scene (not extractable as code)` section naming the engine (`three.js r160`, Rive, PixiJS, Babylon.js) and the rendered size, plus a plain statement that a runtime-drawn scene has no HTML or CSS to capture and what to do instead. A WebGL hero used to capture as an unexplained empty box.
- **anime.js animations read** (#90): an `## anime.js` section lists each running instance touching the picked subtree — the animated properties, duration, easing, delay, direction and loop. Partial by nature (anime.js does not expose its keyframes), but enough to describe the motion instead of leaving it out.
- **Lottie animations captured as their JSON** (#89): a `## Lottie` section names each animation on the picked subtree — frames, frame rate, duration, loop — and inlines its `animationData`, which is the complete, portable animation and replays identically anywhere you paste it. This is the one animation format on the web where a full extraction is genuinely possible. JSON over 100 KB is summarised (layer and asset counts) instead of inlined. Detected through `<lottie-player>`, `<dotlottie-player>` and the `lottie-web` registry.
- **Animation cards with a drawn easing curve** (#87): `cubic-bezier(0.4, 0, 0.2, 1)` means nothing on sight; the curve does. Expanding **Animations** draws each running animation's easing as a small SVG — a cubic for `cubic-bezier` and the named easings, a staircase for `steps(n)`, a straight line for `linear` — with the duration and iteration count as the caption. Capped at 8 cards, then `+N more`.
- **Type specimens and spacing bars** (#86): expanding **Fonts** renders each captured style as an `Aa` in that family, weight and size (capped at 28px so a hero headline doesn't take the drawer with it) beside its metrics; **Palette** now also draws its spacing values as proportional bars, so a 4px or 8px grid is a shape you see rather than a list of numbers to check.
- **Colour swatches and token chips** (#85): expanding the **Palette** section in the drawer draws its colours as chips instead of quoting them, and **Tokens** as name + swatch + value rows. Click a chip to copy the exact colour, or a token row to copy `var(--name)`. Translucent colours are drawn over a checkerboard so the alpha is visible. A `text` link on the row flips back to the raw Markdown, which is still what gets pasted.
- **Select all / none, and a selection remembered per site** (#83): `all · none` on the Sections heading tick or clear everything at once, and the selection you used on a site is what you get back next time you capture there — kept for the 50 most recently used hosts. Sections a site's old selection named that the new capture does not have are dropped silently.
- **Prompt, live output box and Copy selected** (#82): above the section rows the drawer now has a one-line prompt and a read-only preview of exactly what a copy would produce — prompt plus the ticked sections, updating as you type or tick. **Copy selected** puts that on the clipboard and reports how many sections and how many KB went with it, so a targeted paste replaces the 400 KB wall. The prompt is remembered between drawer opens; sections always assemble in capture order, never in the order you clicked them.
...
### Fixed
- **The self-check was reading `picker.js` as windows-1252.** `test/fixture.html` declared no charset, so on a `file://` page Chrome decoded the bundled script with the legacy fallback encoding and every em dash in it became `â€"` — including the ones the new section parsers match on. The fixture's own expected strings were mis-decoded identically, so the assertions still passed while the parsers silently matched nothing against real captured text. The fixture now declares UTF-8, and a guard asserts that every section with a visual form produces one from a real capture rather than from a literal typed in the test. (Chrome always serves extension resources as UTF-8; the shipped extension was never affected.)
